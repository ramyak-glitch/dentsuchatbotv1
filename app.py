"""
RK- Dentsu Smart Buddy — v5.1 (Production)
=========================================
Features:
...
def convert_pdf_to_ppt(pdf_file, output_path="output.pptx"):
    """
    Convert uploaded PDF into a PPT by extracting text and creating slides.
    Each page / chunk becomes one slide.
    """
    try:
        import pymupdf
        ...
        for i, page in enumerate(doc):
            ...
            # Split large content
            content.text = text[:1000]  # avoid overflow (optional tuning)
        ...
        if uploaded:
            for uf in uploaded:
                if uf.name.endswith(".pdf"):
                    if st.button(f"Convert {uf.name} to PPT"):
                        with st.spinner("Converting PDF to PPT..."):
                            output_file = f"{uf.name.replace('.pdf','')}.pptx"
                            result = convert_pdf_to_ppt(uf, output_file)
